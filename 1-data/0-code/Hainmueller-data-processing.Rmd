...
資料來自 [Hainmueller et al. (2014)](https://doi.org/10.1093/pan/mpt024) 的研究，
且有[公開資料](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/THJYQR)。
此研究共有 candidate experiment 和 immigrant experiment 兩種實驗和對應的資料。
...
profiles.candidate <- expand.grid(
  MilitaryService = names(attr(candidate, "label.table")[[2]]),
  ...
  RaceEthnicity = names(attr(candidate, "label.table")[[9]]),
...
candidate.dat <- candidate %>% 
  rename(any_of(colname.candidate)) %>%  # Name the attribute codings
  rename_with(~ gsub("([ /])", "", .x)) %>%  # make the column name camel cases
  left_join(profiles.candidate, colnames(profiles.candidate)[-1]) %>% 
  mutate(trial_id = ceiling(row_number() / 2)) %>% 
  rename_with(~ paste0(.x, "."), .cols=2:9) %>%   # make attribute names end with "."
  relocate(c(trial_id, profile_id, select), .after = 1)  # let trial_id and profile_id be top columns
...
profiles.immigrant <- expand.grid(
  ...
  ApplicationReason = names(attr(immigrant, "label.table")[[5]]),
  Profession = names(attr(immigrant, "label.table")[[9]]),
  JobExperience = names(attr(immigrant, "label.table")[[6]]),
  JobPlans = names(attr(immigrant, "label.table")[[7]]),
  PriorTripstoUS = names(attr(immigrant, "label.table")[[2]]),
  Language = names(attr(immigrant, "label.table")[[8]])
) %>% 
  mutate(profile_id = 1:nrow(.), .before=1) %>% 
  filter(
    (!Profession %in% highly_skilled_job) | (Education %in% have_college_education),
    (ApplicationReason != "Escape political/religious persecution") | (Origin %in% danger_country)
  )
...
immigrant.dat <- immigrant %>% 
  ...
  rename_with(~ gsub("([ /])", "", toTitleCase(.x)), .cols=3:11) %>%  # make the column name camel cases
  left_join(profiles.immigrant, colnames(profiles.immigrant)[-1]) %>% 
  mutate(
    trial_id = ceiling(row_number() / 2),
    select = (select == "Yes") * 1
  ) %>% 
  rename_with(~ paste0(.x, "."), .cols=3:11) %>%   # make attribute names end with "."
  relocate(c(trial_id, profile_id, select), .after = 1)  # let trial_id and profile_id be top columns
